...
import { SendHorizontal, ArrowLeft, Smile, Video, MoreVertical,Cpu, CheckCheck  } from "lucide-react";
...
export default function AIChatPage() {
  ...
  const handleReceiveAIMessage = useCallback((message) => {
...
const handleSendMessage = (text) => {
    ...
    socket.emit("send_ai_message", messageData);
    setMessages((prev) => [...prev, { ...messageData, _id: Date.now(), sender: authUser }]);
    setMessageText("");
    setShowInputEmoji(false);
  };
  ...
  return (
    <div className={`w-full min-h-[100dvh] flex flex-col ${theme.bg} font-sans`}>
      {/* Header */}
      <header className={`fixed top-35 left-0 right-0 z-40 px-4 py-3 ${theme.headerBg} ${theme.headerBorder} border-b shadow-sm`}>
    ...
    <main className={`flex-1 px-4 sm:px-5 py-5 mt-[50px] mb-[60px] ${theme.chatBg}`}>
                  ...
                  {editingMessageId === msg._id ? (
                    <div className="flex gap-2">
                      ...
                      >
                        ...
                        <div className={`p-2 mb-2 rounded ${isSender ? theme.parentsender : theme.parentreceiver}`}>
                          <p className="text-sm">{msg.text}</p>
                          <div className={`flex items-center mt-1 text-xs text-gray-400 gap-1 ${isSender ? "justify-end" : "justify-start"}`}>
                            <span>
